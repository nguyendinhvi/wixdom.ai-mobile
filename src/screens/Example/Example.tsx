...
const Example = () => {
  ...
  const [numberRandom, setNumberRandom] = useState(2);
  ...
  useEffect(() => {
    ExampleService.get(numberRandom)
      .then(res => {
        setData(res);
      })
      .catch(error => {
  ...
  const nextHello = () => {
    setNumberRandom(Math.ceil(Math.random() * 10 + 1));
    {
      data?.name && Alert.alert(t('example:helloUser', { name: data.name }));
    }
  };
